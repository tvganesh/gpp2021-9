...
#########################################################################################################
# Analyze IPL bowlers
analyzeBowlers <- function(bowler,func, t20type="IPL",dateRange, staticIntv1=1) {
  ...
  cat("bowler=",bowler,"\n")
    ...
    if(func == "Mean Economy Rate of bowler"){
        bowlerMeanEconomyRate(bowlerDF,bowler,dateRange,staticIntv1)
    } else if (func == "Mean runs conceded by bowler"){
        bowlerMeanRunsConceded(bowlerDF,bowler,dateRange,staticIntv1)
    }     else if (func == "Bowler's Moving Average"){
        bowlerMovingAverage(bowlerDF,bowler,dateRange,staticIntv1)
    } else if (func == "Bowler's Cumulative Avg. Wickets"){
        bowlerCumulativeAvgWickets(bowlerDF,bowler,dateRange,staticIntv1)
    } else if (func == "Bowler's Cumulative Avg. Economy Rate"){
        bowlerCumulativeAvgEconRate(bowlerDF,bowler,dateRange,staticIntv1)
    } else if (func == "Bowler's Wicket Plot"){
        bowlerWicketPlot(bowlerDF,bowler,dateRange,staticIntv1)
    } else if (func == "Bowler's Wickets against opposition"){
        bowlerWicketsAgainstOpposition(bowlerDF,bowler,dateRange,staticIntv1)
    } else if (func == "Bowler's Wickets at Venues"){
        bowlerWicketsVenue(bowlerDF,bowler,dateRange,staticIntv1)
      ...
      bowlerWktsPredict(bowlerDF1,bowler,dateRange)
